...
11a. Before billing activation, record CPA approval of Nevada tax treatment and verify zero unexplained reconciliation differences. Set the live/restricted live key, matching account ID, live webhook secret, and `STRIPE_EXPECTED_MODE=live` while keeping `STRIPE_INVOICING_ENABLED=false`; redeploy and verify configured/live/live/disabled through `/api/health`.
11b. In a supervised window, set `STRIPE_INVOICING_ENABLED=true`, redeploy, and issue/pay/reconcile one legitimate low-value live invoice for a completed job. Inspect successful webhook delivery and duplicate replay, confirm the application ledger and Stripe agree, and verify payout scheduling. Update the production smoke expectations to live/enabled in the same approved release. After a live object exists, rollback disables invoice sending but retains all live Stripe credentials and the live webhook; production must never be pointed back to test mode.
