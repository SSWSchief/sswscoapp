...
- Have a CPA, Nevada sales-tax adviser, or the Nevada Department of Taxation document Nevada sales/use-tax treatment; the absence of Nevada individual income tax is not a sales-tax determination. If no qualified review exists, record `follow_up_required` with the date and reason and keep live sending blocked. If any invoice item is taxable, stop activation until Stripe Tax or reviewed manual tax-line behavior is approved and implemented. Record an approved non-taxable decision only with `npm run invoices:tax-policy -- --apply --status=non_taxable_approved --note="<approver, date, advice>" --confirm-production --project-ref=<ref>`; the settings screen shows the gate but deliberately cannot change it, and the write is audited. Run it with no arguments first to read the current state.
...
- Record the pre-existing unmatched test events as stale test artifacts; do not import them into the application ledger.
- After account-owner approval, switch production to a live/restricted live key, the matching account ID and live webhook secret, set `STRIPE_EXPECTED_MODE=live`, and keep `STRIPE_INVOICING_ENABLED=false` for the first redeploy. Confirm `/api/health` reports configured/live/live/disabled before allowing any money action.
- In a supervised window, set `STRIPE_INVOICING_ENABLED=true`, redeploy, and issue one legitimate low-value invoice for a completed job. Pay it by card, verify the signed webhook changes the local ledger to paid, replay the event to prove duplicate handling, reconcile it, and confirm the payment is scheduled for payout. Leave sending enabled only when every check passes.
- After the first live invoice exists, rollback means disabling `STRIPE_INVOICING_ENABLED` and redeploying while retaining the live key, account ID, and live webhook. Never reconnect the production database to Stripe test mode. Preserve rollback evidence.
