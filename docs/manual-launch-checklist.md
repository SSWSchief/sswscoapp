...
- **Custom SMTP is connected (August 16, 2026) and this item is closed.** Mail goes out through Resend on the verified `sswsco.com` domain, and `NEXT_PUBLIC_EMAIL_DELIVERY_ENABLED=true` in Vercel production, so both onboarding paths work: administrator-issued temporary passwords, or an emailed invitation. This bullet previously said SMTP was deferred and the flag stayed `false`; that was true when written and is no longer, which made it worth correcting rather than leaving to be discovered against a live deployment. The Site URL and redirect items above still apply and are the standing regression risk. See the appendix in `docs/client-handoff.md` for what is actually configured, including the template form that must **not** be hand-edited.
