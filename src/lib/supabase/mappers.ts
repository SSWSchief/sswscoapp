...
export function mapCustomer(row: CustomerRow, activeJobs = 0): Customer {
  return {
    id: row.id,
    name: row.name,
    phone: row.phone,
...
export function mapTruck(row: TruckRow): Truck {
  return {
    ...
    registrationDueDate: row.registration_due_date ?? "",
    mileage: row.mileage,
    lastPmDate: row.last_pm_date ?? "",
    lastPmMileage: row.last_pm_mileage,
    nextPmDate: row.next_pm_date ?? "",
    nextPmMileage: row.next_pm_mileage,
    make: row.make,
    model: row.model,
    vin: row.vin,
    ...
    notes: row.notes,
    airTagId: row.air_tag_id,
    gpsSource: row.gps_source ?? undefined,
...
export function mapJob(
  ...
  return {
    ...
    dumpsterSize: row.dumpster_size as Job["dumpsterSize"],
    assignedDriverId: row.assigned_driver_id,
    ...
    photos: photos
      ...
      .map((p) => ({
        ...
        jobId: p.job_id,
        url: p.url,
        uploadedById: p.uploaded_by_id,
        createdAt: p.created_at,
      })),
    timeline: events
      .filter((e) => e.job_id === row.id)
      .map((e) => ({
        type: e.event_type as Job["timeline"][number]["type"],
        at: e.occurred_at,
      })),
  };
}
...
export function mapActivity(row: JobActivityRow): JobActivity {
  return {
    id: row.id,
    jobId: row.job_id,
    actorId: row.actor_id,
    actorName: row.actor_name,
    type: row.activity_type as JobActivity["type"],
    body: row.body,
    createdAt: row.created_at,
    dispatchNotified: row.dispatch_notified,
  };
}
export function mapJobNote(row: JobNoteRow, authorName = "Employee"): JobNote {
  return {
    id: row.id,
    jobId: row.job_id,
...
export function mapTimeCorrection(row: CorrectionRow): TimeEntryCorrection {
  return {
    id: row.id,
...
export const mapPretripTemplate = (
  row: PretripTemplateRow,
): PretripTemplate => ({
  id: row.id,
  title: row.title,
  version: row.version,
  isPublished: row.is_published,
  items: row.items as unknown as PretripTemplate["items"],
});
export const mapPretripSubmission = (
...
export const mapSopDocument = (
  ...
  body: row.body,
  isPublished: row.is_published,
  requiredForDrivers: row.required_for_drivers,
  createdAt: row.created_at,
  acknowledged,
});
export const mapCompanySettings = (
