    ...
    <>
      ...
      <div className="portal-content space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <h2 className="font-heading text-3xl font-bold uppercase tracking-wide text-brand-charcoal">
            {truck.number}
          </h2>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <Card>
            ...
            <dl className="p-5 space-y-4">
              ...
              <Detail
                ...
                value={formatDate(truck.registrationDueDate)}
              />
            </dl>
          </Card>
          ...
          <Card>
            ...
            <dl className="p-5 space-y-4">
              ...
              <Detail
                label="Last PM"
                value={`${formatDate(truck.lastPmDate)} · ${truck.lastPmMileage.toLocaleString()} mi`}
              />
              <Detail
                label="Next PM Date"
                value={formatDate(truck.nextPmDate)}
              />
              <Detail
                label="Next PM Mileage"
                value={`${truck.nextPmMileage.toLocaleString()} mi`}
              />
            </dl>
            <div className="mx-5 mb-5 rounded border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">
              Mileage is maintained manually and can be captured with each
          ...
          <Card>
            ...
            <dl className="p-5 space-y-4">
              ...
              <Detail
                ...
                value={truck.lastKnownLocation ?? "—"}
              />
              <Detail
                label="Last Seen"
                value={
                  truck.lastSeenAt ? formatDateTime(truck.lastSeenAt) : "—"
                }
              />
...
function Detail({
  ...
  return (
    <div>
      <dt className="font-heading text-xs font-semibold uppercase tracking-wide text-brand-steel">
        {label}
      </dt>
      <dd
        className={`mt-1 break-words text-sm font-medium text-brand-charcoal ${mono ? "font-mono" : ""}`}
      >
        {value}
      </dd>
    </div>
