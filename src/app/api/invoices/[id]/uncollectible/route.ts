...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const api = await authorizedInvoiceApi(request, "/api/invoices/[id]/uncollectible", "POST"); if (api.denied) return api.denied;
  try { const { id } = await params; return api.success(await transitionInvoice(id, "uncollectible")); }
