...
for (const owner of owners) {
  ...
  if (authUser) {
    ...
    if (updated.error)
      throw new Error(`${owner.email}: ${updated.error.message}`);
    ...
    if (created.error)
      throw new Error(`${owner.email}: ${created.error.message}`);
  ...
  if (existingProfile.error)
    throw new Error(`${owner.email}: ${existingProfile.error.message}`);
  ...
  const profile = existingProfile.data
    ...
    : await service
        ...
        .single();
  if (profile.error)
    throw new Error(`${owner.email}: ${profile.error.message}`);
